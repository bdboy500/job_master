...
export default function Home() {
  ...
  useEffect(() => {
    if (currentScreen !== "quiz" || !quizStarted || loading || isSubmitted || isTimedOut || isCompleted || showQuitConfirmModal) return;
    ...
    const interval = setInterval(() => {
      setTimeLeft((prev) => {
        ...
        const next = prev - 1;
        // Countdown sound alert for the full 30 seconds (30 to 1)
        if (next <= 30 && next >= 1 && soundEnabled) {
          quizAudio.playCountdownTick(next <= 5);
        }
        return next;
      });
    }, 1000);
    ...
    return () => clearInterval(interval);
  }, [currentScreen, quizStarted, loading, isSubmitted, isTimedOut, currentQuestionIndex, isCompleted, showQuitConfirmModal, soundEnabled]);
  ...
  // Handle start quiz action (Immediately navigates to quiz screen with skeleton loader)
  const startQuizFlow = (title: string, subtitle: string, customQuestionSet?: Question[]) => {
    if (soundEnabled) quizAudio.playClick();

    // Check offline beforehand for live quiz
  ...
  const handleRestart = async () => {
    ...
    // Immediately show quiz screen with skeleton loader!
    setLoading(true);
    setQuizStarted(true);
    setCurrentQuestionIndex(0);
    setScore(0);
        ...
        <div className="flex-1 overflow-y-auto overscroll-y-auto pb-2 md:pb-0 bg-slate-50/60 relative touch-pan-y md:px-20 lg:px-48 xl:px-72 2xl:px-96">
          ...
          {currentScreen === "quiz" && (
            <div className="p-4 sm:p-5 space-y-4 sm:space-y-5 animate-fade-in text-left">
              ...
              {/* TOP HUD (ss2 Style): Left Correct Count & Green Pill | Center Circular Countdown | Right Wrong Count & Red Pill */}
              {(() => {
                ...
                return (
                  <div className="bg-white border border-slate-200/80 rounded-3xl p-3 sm:p-4 shadow-xs flex items-center justify-between px-4 sm:px-6">
                    {/* Left: Correct Indicator (Number first, then Green Pill) */}
                    <div className="flex items-center gap-2">
                      <span className="font-mono font-black text-base sm:text-lg text-emerald-600 leading-none">
                        {score < 10 ? `0${score}` : score}
                      </span>
                      <span className="w-6 sm:w-8 h-2 sm:h-2.5 rounded-full bg-emerald-500 shadow-xs inline-block" />
                    </div>
              ...
              {loading && (
                <div className="bg-white border border-slate-200/80 rounded-3xl p-5 sm:p-6 shadow-sm space-y-4 sm:space-y-5 animate-fade-in">
                  {/* Top Row: Question Count Skeleton + Live Quiz Skeleton */}
                  <div className="flex items-center justify-between">
                    <div className="h-7 w-28 rounded-xl bg-orange-100/70 animate-shimmer" />
                    <div className="h-7 w-24 rounded-full bg-rose-100/80 border border-rose-200/60 animate-shimmer" />
                  </div>

                  {/* Question Title Skeleton (2 lines) */}
                  <div className="space-y-2 pt-1">
                    <div className="h-5 w-11/12 bg-slate-200/80 rounded-lg animate-shimmer" />
                    <div className="h-5 w-3/4 bg-slate-200/80 rounded-lg animate-shimmer" />
                  </div>

                  {/* Options List Skeleton (4 Options) */}
                  <div className="space-y-2.5 sm:space-y-3 pt-1">
                    {[1, 2, 3, 4].map((idx) => (
                      <div
                        key={idx}
                        className="w-full p-3.5 sm:p-4 rounded-2xl border-2 border-slate-100 bg-slate-50/50 flex items-center justify-between gap-3"
                      >
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          <div className="w-7 h-7 rounded-xl bg-orange-200/70 shrink-0 animate-shimmer" />
                          <div
                            className={`h-4 rounded-md bg-slate-200 animate-shimmer ${
                              idx === 1 ? "w-3/5" : idx === 2 ? "w-4/5" : idx === 3 ? "w-1/2" : "w-2/3"
                            }`}
                          />
                        </div>
                      </div>
                  ...
                  <div className="pt-2">
                    <div className="w-full h-13 rounded-2xl bg-gradient-to-r from-orange-200 via-amber-100 to-orange-200 animate-shimmer" />
                  </div>

                  {/* Bottom Question Count Indicator Skeleton */}
                  <div className="pt-2.5 border-t border-slate-100 flex items-center justify-center">
                    <div className="h-4 w-48 rounded-md bg-slate-100 animate-shimmer" />
                  </div>
                </div>
              )}
              ...
              {!loading && quizStarted && isTimedOut && (
                <div className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 text-center flex flex-col items-center gap-4 shadow-md animate-fade-in">
                  <div className="w-14 h-14 bg-red-50 text-red-500 rounded-2xl flex items-center justify-center border border-red-100 animate-bounce">
                  ...
                  <div className="grid grid-cols-2 gap-3 w-full pt-1">
                    <button
                      ...
                      className="py-3.5 px-4 bg-[#FF6A00] hover:bg-[#E54800] text-white font-extrabold text-xs rounded-2xl active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-sm cursor-pointer"
                    >
              ...
              {/* Completed Screen */}
              {!loading && isCompleted && (
                <div className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 text-center flex flex-col items-center gap-5 shadow-md">
                  <div className="w-16 h-16 bg-orange-50 text-[#FF6A00] rounded-3xl flex items-center justify-center border border-orange-200 shadow-2xs">
                    <Award className="w-8 h-8" />
                  </div>
                  <div className="space-y-1.5">
                    <h3 className="font-black text-xl sm:text-2xl text-slate-900">অভিনন্দন! কুইজ সম্পন্ন</h3>
                    <p className="text-slate-500 text-xs leading-relaxed max-w-[280px] mx-auto">
                      {score === questions.length 
                        ? "চমৎকার! আপনি সবকটি প্রশ্নের শতভাগ সঠিক উত্তর দিয়েছেন। 🏆" 
                        : score >= questions.length / 2 
                        ? "দুর্দান্ত চেষ্টা! আপনার প্রস্তুতি আরও জোরদার করতে নিয়মিত কুইজ খেলুন। 🎉" 
                        : "ভালো চেষ্টা! নিয়মিত কুইজ প্র্যাকটিস করে স্কোর বৃদ্ধি করুন।"}
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 w-full">
                    ...
                    <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-3 text-center">
                      <span className="text-[10px] font-extrabold text-emerald-600 uppercase block">সঠিক উত্তর</span>
                    ...
                    <div className="bg-rose-50 border border-rose-100 rounded-2xl p-3 text-center">
                      <span className="text-[10px] font-extrabold text-rose-600 uppercase block">ভুল উত্তর</span>
                      <span className="text-base font-black text-rose-700">{Math.max(0, submittedCount - score)}</span>
                    </div>
                  </div>

                  <div className="w-full bg-orange-50/80 border border-orange-200/80 rounded-2xl p-3.5 flex justify-between items-center text-xs font-extrabold text-slate-700">
                    <span>সঠিক উত্তর হার (Accuracy)</span>
                    <span className="text-[#FF6A00] font-mono font-black text-lg">
                  ...
                  <div className="grid grid-cols-2 gap-3 w-full">
                    <button
                      onClick={handleRestart}
                      className="py-3.5 bg-[#FF6A00] hover:bg-[#E54800] text-white font-extrabold rounded-2xl active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-sm cursor-pointer text-xs"
                    >
              ...
              {/* Active Quiz Card */}
              {quizStarted && !isTimedOut && !isCompleted && !loading && currentQuestion && (
                <div className="bg-white border border-slate-200/80 rounded-3xl p-5 sm:p-6 shadow-sm space-y-4 sm:space-y-5">
                  
                  {/* Top Question Header: Question index badge & Animated Red Live Quiz Badge */}
                  <div className="flex items-center justify-between">
                    <span className="bg-orange-50 text-[#FF6A00] border border-orange-200/90 font-black text-xs px-3 py-1 rounded-xl">
                      প্রশ্ন {currentQuestionIndex + 1} / {questions.length}
                    </span>
                    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-rose-50 border border-rose-200/80 shadow-2xs">
                    ...
                    {currentQuestion.options.map((option, index) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left p-3.5 sm:p-4 rounded-2xl text-sm sm:text-base flex items-center justify-between gap-3 transition-all ${containerStyle} ${
                  ...
                  {/* Action row (Submit / Next button) */}
                  <div className="pt-2">
                    <button
                      onClick={handleNext}
                      disabled={!isSubmitted}
                      className={`w-full py-4 px-6 font-black rounded-2xl text-xs sm:text-sm tracking-wider uppercase transition-all flex items-center justify-center gap-2 ${
                        isSubmitted
                          ? "bg-gradient-to-r from-[#FF6A00] to-[#E54800] hover:from-[#FF5500] hover:to-[#CC4000] text-white shadow-md shadow-orange-500/25 active:scale-[0.98] cursor-pointer"
                    ...
                    >
                      {currentQuestionIndex + 1 >= questions.length ? (
                        ...
                        <>
                          <span>{isSubmitted ? "পরবর্তী প্রশ্ন" : "একটি উত্তর নির্বাচন করুন"}</span>
              ...
              <div className="flex items-center justify-between px-1 pt-1">
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-full bg-[#FF6A00] animate-pulse"></span>
          ...
          {currentScreen === "prep-sub-detail" && selectedPrepSubSubject && (
            <div className="p-5 space-y-5 animate-fade-in pb-10 text-left">
              ...
              {/* CASE 1: No Section Selected -> Show Section Cards Grid */}
              {!selectedPrepExamTypeFilter && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between px-1">
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              ...
                              <div className="w-8 h-8 rounded-full flex items-center justify-center transition-colors bg-slate-50 text-slate-400 group-hover:bg-orange-50 group-hover:text-[#FF6A00]">
                                <ChevronRight className="w-4 h-4 stroke-[2.5px]" />
                              </div>
                            </div>
                          </div>
              ...
              {/* CASE 2: Inside Selected Exam Section -> Show Navigation Header & Section Question Papers */}
              {selectedPrepExamTypeFilter && (
                <div className="space-y-4 pt-1">
                  {/* Top Bar inside Section */}
                  <div className="flex items-center justify-between bg-white border border-slate-200/90 rounded-2xl p-3 shadow-2xs">
                    <button
                      ...
                      className="flex items-center gap-1.5 text-xs font-black text-[#FF6A00] bg-orange-50 hover:bg-orange-100 px-3.5 py-2 rounded-xl border border-orange-200 transition-all cursor-pointer active:scale-95 shadow-2xs"
                    >
                      <ArrowLeft className="w-4 h-4 stroke-[2.5px]" />
                      <span>অন্যান্য সেকশন</span>
                    </button>
                        ...
                        return (
                          <div className="bg-white border border-slate-200/80 rounded-[2rem] p-8 text-center space-y-3 shadow-2xs">
                            ...
                            <button
                              ...
                              className="px-4 py-2 bg-[#FF6A00] text-white text-xs font-black rounded-xl active:scale-95 transition-all cursor-pointer shadow-sm shadow-orange-500/20 inline-block mt-1"
                            >
                              অন্যান্য সেকশন দেখুন
                            </button>
                          </div>
